...
export function ContestArena() {
  ...
  const finish = useMutation({
    ...
    onSuccess: () => navigate(`/app/contests/${publicId}/leaderboard`),
  });
      ...
      <div className="flex h-full min-h-0 flex-col bg-bg-subtle lg:h-[calc(100vh-4rem)] lg:overflow-hidden">
        ...
        <div className="flex h-12 shrink-0 items-center justify-between gap-3 border-b border-line bg-surface px-4">
          <Link
            ...
            title="Your timer keeps running — Finish & submit to lock in your attempt"
          ...
          <div className="flex items-center gap-3">
            {attemptActive && c?.attemptEndsAt ? (
              <span className="inline-flex items-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-[12px] font-medium text-emerald-600 dark:text-emerald-400">
                ...
                <Countdown
                  target={c.attemptEndsAt}
                  onElapsed={() => navigate(`/app/contests/${publicId}/leaderboard`)}
                />
              </span>
            ) : null}
            <Link
